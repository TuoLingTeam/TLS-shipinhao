...
mod tests {
    ...
    fn fresh_setup() -> (TempDir, PathBuf, ManifestPayload, SigningKey, String) {
        ...
        let hash_b = write_file(dir.path(), "ui/dist/index.html", b"<html></html>");
    ...
    fn tampered_file_byte_reports_file_modified() {
        ...
        // 改 ui/dist/index.html 一个字节
        std::fs::write(dir.path().join("ui/dist/index.html"), b"<html>X</html>").unwrap();
        let err = validate_runtime_continuity(&manifest_path, &vk).unwrap_err();
        match err {
            IntegrityError::FileModified(path) => assert_eq!(path, "ui/dist/index.html"),

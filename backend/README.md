...
**正式部署入口为本目录**：在 `backend/` 执行 `npx wrangler deploy`（`wrangler.toml` 会编译当前 backend crate，并指向 `build/worker/shim.mjs`）。

- **管理后台页面**唯一源文件：`backend/assets/admin.html`（由 Rust Worker 在编译期 `include_str!` 嵌入）。
...
客户端内置的验签公钥常量在 `backend/src/license/service.rs` 的 `LICENSE_PUBLIC_KEY_B64`。**轮换签名私钥前，必须同步更新该常量与所有已发布客户端**，否则旧设备的 Lease Token 将全部进入 `LicenseState::Invalid`，被迫重新激活。
...
（`wrangler` 会在部署前执行 `[build]`，在 `backend/` 内运行 `worker-build`。）
...
支持的 `task_type` 白名单在 `backend/src/contracts.rs` 的 `SUPPORTED_TASKS`：`review_find` / `review_full_scan` / `quality_refund` / `batch_delivery` / `cache_manage`。

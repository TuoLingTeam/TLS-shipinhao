...
async function doLogin() {
  ...
  if (!SECRET) { showMsg("loginMsg", "请输入管理密钥", true); return; }
  const res = await api("/api/admin/list", {});
  if (!res.success) { showMsg("loginMsg", "密钥错误", true); SECRET = ""; return; }
...
async function doGenerate() {
  ...
  const res = await api("/api/admin/generate", { count, plan_days, note });
...
document.getElementById("adminSecret").addEventListener("keydown", e => { if (e.key === "Enter") doLogin(); });
</script>
</body>
</html>`;

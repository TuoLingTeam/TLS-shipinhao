...
//! # 设计理由
//!
//! 原版 Python (backup/legacy-src/app/services/review_matcher.py) 的候选收集
//! 按 `product_id+sku_id` 建索引：先拉出同 SKU 的所有订单，再靠昵称相似度
//! 评分排序挑出真匹配。这套设计在长尾商品（单 SKU 订单数少）场景没问题，
//! 但一旦遇到爆品（单 SKU 数千甚至上万订单）就暴露出两个硬伤：
//!

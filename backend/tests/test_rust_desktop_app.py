...
class RustDesktopAppTests(unittest.TestCase):
    def test_desktop_app_uses_slint_and_declares_main_window(self):
        cargo_toml = ROOT / "backend" / "crates" / "desktop-app" / "Cargo.toml"
        main_rs = ROOT / "backend" / "crates" / "desktop-app" / "src" / "main.rs"
        slint_file = ROOT / "backend" / "crates" / "desktop-app" / "ui" / "app-window.slint"
    ...
    def test_desktop_app_contains_real_shell_bridge(self):
        shell_rs = ROOT / "backend" / "crates" / "desktop-app" / "src" / "app_shell.rs"

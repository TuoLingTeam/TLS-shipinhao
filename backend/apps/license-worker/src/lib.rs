...
mod cloudflare_entry {
    use super::*;
    use crate::runtime::repo_d1::D1RuntimeRepo;
    ...
    async fn route_fetch(mut req: Request, env: Env) -> Result<Response> {
        ...
        if method == Method::Get && path == "/admin" {
            return crate::admin::d1::serve_admin_html().await;
        }

        if path.starts_with("/api/admin/") {
            return crate::admin::d1::handle_admin_request(req, &env).await;
        }
        ...
        if route == WorkerRoute::LeaseRevoke {
            if let Some(resp) = crate::admin::d1::check_admin(req.headers(), &env)? {
                return Ok(resp);
            }
        }

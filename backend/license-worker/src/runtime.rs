...
//! - [`D1RuntimeRepo`]（仅 `wasm32` target 下编译的 Cloudflare D1 实现）
//!
//! 都收敛到这里。`lib.rs` 只保留三件事：[`crate::LeaseTokenSigner`]（签发工
//! 具）、`cloudflare_entry`（wasm `fetch` 事件入口）以及对 `messages` 与本
//! 模块的 `pub use` 转发，保证外部测试与调用方的 import 路径（`use

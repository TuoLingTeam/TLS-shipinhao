...
DEFAULT_WINDOW_HEIGHT = 790       # 非 Windows 系统默认窗口高度
WINDOWS_DEFAULT_WINDOW_WIDTH = 960  # Windows 系统默认窗口宽度
WINDOWS_DEFAULT_WINDOW_HEIGHT = 840  # Windows 系统默认窗口高度
...
INPUT_VISIBLE_LINES = 25           # 输入框可见行数
...
ORDER_WINDOW_INITIAL_COUNT = 3          # 初始订单时间分片数量
ORDER_WINDOW_SPLIT_PAGE_THRESHOLD = 80  # 单窗口页深达到该值后触发切分
ORDER_WINDOW_MIN_SECONDS = 6 * 3600     # 最小时间窗口宽度（6 小时）
ORDER_WINDOW_MAX_DEPTH = 8              # 时间窗口最大切分深度
ORDER_RISK_WINDOW_WORKERS = 1           # 风控降级模式下的窗口并发数
ORDER_RISK_SPLIT_PAGE_THRESHOLD = 20    # 风控降级模式下的页深切分阈值
ORDER_RISK_MIN_SECONDS = 15 * 60        # 风控降级模式下的最小时间窗口宽度（15 分钟）

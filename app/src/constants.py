...
# 窗口 & 品牌
APP_VERSION = "4.2"                # 应用版本号（Mac 简介/Windows 属性及窗口标题）
WINDOW_TITLE = f"驼铃·视频小店差评处理 {APP_VERSION}"  # 窗口标题
...
DEFAULT_WINDOW_HEIGHT = 830       # 默认窗口高度（Mac/Windows 统一）
...
# 3. 昵称与商品均完全一致时得分 100。
# 4. 一边完全一致、另一边按相似度档位扣分。
# 5. 分数最低封底 50。
MATCH_MIN_SCORE = 50              # 达到该分数才认为"可匹配"
AUTO_FILL_SCORE_THRESHOLD = 100   # 达到该分数才自动填入订单号，低于该分数需要人工核对
